...
class Validator {
    ...
    companion object {
        ...
        private val NAME_VALIDATION_MSG = "Please enter a valid name"
        private val BANK_VALIDATION_MSG = "Select a bank"
        private val FIRST_NAME_VALIDATION_MSG = "Please enter a valid first name"
        private val LAST_NAME_VALIDATION_MSG = "Please enter a valid last name"
        private val EMAIL_VALIDATION_MSG = "Please enter a valid email address"
        private val PHONE_VALIDATION_MSG = "Please enter a valid phone number"
        private val USERNAME_VALIDATION_MSG = "Please enter a valid username"
        private val ACCOUNT_NUMBER_VALIDATION_MESSAGE = "Please enter 10 digit account number"
        private val PROMOCODE_VALIDATION_MSG = "Promo code must be 10 characters"
        private val COUMTRY_VALIDATION_MSG = "Please enter a valid country"
        private val STATE_VALIDATION_MSG = "Please enter a valid state"
        private val CITY_VALIDATION_MSG = "Please enter a valid city"
        private val POSTAL_CODE_VALIDATION_MSG = "Please enter a postal code"
        private val ADDRESS_VALIDATION_MSG = "Please enter a valid address"
...
//        private fun getNum(data: String): Int {
//            var num = 0
//            if (data is EditText) {
//                num = data.toInt()
//            }
...
//        }
        ...
        fun isValidState(data: Any, updateUI: Boolean = true): Boolean {
            val str = getText(data)
            val valid = str.trim().length > 2

            // Set error if required
            if (updateUI) {
                val error: String? = if (valid) null else STATE_VALIDATION_MSG
                setError(data, error)
            }
        ...
        fun isValidPostalCode(data: Any, updateUI: Boolean = true): Boolean {
            ...
            // Set error if required
            if (updateUI) {
                val error: String? = if (valid) null else POSTAL_CODE_VALIDATION_MSG
                setError(data, error)
            }
         ...
         * @return - true if the email is valid.
         */
        fun isValidPromoCode(data: Any, updateUI: Boolean = true): Boolean {
            val str = getText(data)
            val valid = str.trim().length == 10

            // Set error if required
            if (updateUI) {
                val error: String? = if (valid) null else PROMOCODE_VALIDATION_MSG
                setError(data, error)
            }
